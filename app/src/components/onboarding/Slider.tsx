...
const Slider = ({
  ...
  const left = useVector(0, (HEIGHT / 4) * 3);
  const right = useVector(0, (HEIGHT / 4) * 3);
  ...
  const onGestureEvent = useAnimatedGestureHandler({
    ...
    onEnd: ({velocityX, velocityY, x}) => {
      if (activeSide.value === Side.LEFT) {
        ...
        left.y.value = withSpring((HEIGHT / 4) * 3, {velocity: velocityY});
      } else if (activeSide.value === Side.RIGHT) {
        ...
        right.y.value = withSpring((HEIGHT / 4) * 3, {velocity: velocityY});
      }
    },
  });
